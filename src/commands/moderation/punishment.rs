...
pub async fn send_to_mod_log_channel(
    ctx: Context<'_>,
    ...
    let fields = match punishment.punishment_type {
        PunishmentType::Ban => {
            vec![
                ...
                (
                    "Reason",
                    format!("{}", punishment.reason.clone().unwrap_or("n/a".to_string())),
                    true,
                ),
                (
                    "Deleted messages",
                    format!("{}", punishment.delete_messages.unwrap_or(false)),
                    true,
                ),
            ]
        }
        ...
        PunishmentType::Mute => {
            vec![
                ...
                (
                    "Duration",
                    format!("<t:{}:R>", punishment.duration.unwrap_or(0)),
                    true,
                ),
            ]
        }
