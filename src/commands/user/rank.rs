...
pub async fn rank(ctx: Context<'_>) -> Result<(), Error> {
    let result = download_docs::get_docs(&"commands/user/rank.md").unwrap();
...
pub async fn upload_level_roles(
    ctx: Context<'_>,
    #[description = "upload a file containing your level roles (supported formats: .json, .toml)"]
            ...
            if let serde_json::Value::Object(map) = phrased_json {
                ...
                for (key, value) in map.iter() {
                    ...
                    if !processed_ordering_ids.insert(key) {
                        errors.push(format!(
                            "Error on line {}: Ordering number {} is not unique",
                            index, key
                        ));
                        continue;
                    } else if !processed_role_ids.insert(role_id_number) {
                        errors.push(format!(
                            "Error on line {}: role id {} is not unique",
                            index, role_id_number
                        ));
                        continue;
                    }
                    ...
                    match guild_id
                        .roles(ctx)
                        .await?
                        .get(&RoleId::from(role_id_number as u64))
                    {
                        Some(role) => {
                            if highest_role.position < role.position {
                                errors.push(format!(
                                    "Role with ID {} is higher than the highest role of the bot",
                                    role_id_number
                                ));
                            } else {
            ...
            ctx.send(
                CreateReply::default().embed(
                    CreateEmbed::default()
                        .title("Error")
                        .description(format!(
                            "No valid roles found, encountered error: \n{}",
                            &errors.join("\n---------\n")
                        ))
                        .color(0xff0000),
                ),
            ...
            ctx.send(
                CreateReply::default().embed(
                    CreateEmbed::default()
                        .title("Result")
                        .description(format!(
                            "The following roles have been validated:\n{:?}\n**Errors:**\n{}",
                            &valid_roles,
                            &errors.join("\n---------\n")
                        ))
                        .color(0xffff00),
...
pub async fn set_level_roles(
    ctx: Context<'_>,
    #[description = "assign level roles to specific ranks e.g. (5=role_id,10=role_id) without brackets"]
    roles: String,
) -> Result<(), Error> {
    let guild_id = match ctx.guild_id() {
        Some(id) => id,
    ...
    for (index, role) in roles.split(',').enumerate() {
        ...
        if parts.len() != 2 {
            errors.push(format!(
                "Invalid syntax at position {}: Expected 'ordering=role_id' format",
                index + 1
            ));
            continue;
        }
        ...
        if ordering_number.is_err() || role_id.is_err() {
            errors.push(format!(
                "Invalid values at position {}: Both ordering and role ID must be numeric",
                index + 1
            ));
            continue;
        }
        ...
        if !processed_ordering_ids.insert(ordering_number) {
            errors.push(format!(
                "Duplicated index key {} at position {}",
                ordering_number,
                index + 1
            ));
            continue;
            ...
            errors.push(format!(
                "Duplicated role ID {} at position {}",
                role_id,
                index + 1
            ));
            continue;
        ...
        match guild_id
            .roles(ctx)
            .await?
            .get(&RoleId::from(role_id as u64))
        {
            Some(role) => {
                if highest_role.position < role.position {
                    errors.push(format!(
    ...
    if errors.is_empty() {
        ctx.send(
            CreateReply::default().embed(CreateEmbed::default().title("Result").description(
                format!("All roles validated successfully: \n{:?}", &valid_roles),
            )),
        )
        .await?;
    } else {
        ...
        if valid_roles.is_empty() {
            ...
            ctx.send(CreateReply::default().embed(
                CreateEmbed::default().title("Result").description(format!(
                    "The following roles have been validated:\n{:?}\n**Errors:**\n{}",
                    &valid_roles,
                    &errors.join("\n---------\n")
                )),
                            ...
                            match db.update_level_roles(guild_id.try_into()?, lvl_roles).await {
                                Ok(_) => {
                                    msg.edit(
                                        ctx,
                                        CreateReply::default()
                                            .content("Updated level roles successfully")
                                            .components(vec![]),
                                    )
                                    .await?;
                                }
...
pub async fn set_rank(ctx: Context<'_>, user: User, rank: u16) -> Result<(), Error> {
    ...
    ctx.send(CreateReply::default().embed(
        CreateEmbed::default().title(format!("{}'s rank has been set to {}", user.name, rank)),
    ))
    .await?;
...
#[poise::command(prefix_command, slash_command)]
pub async fn get_rank(ctx: Context<'_>, user: Option<User>) -> Result<(), Error> {
    let guild_id = match ctx.guild_id() {
        Some(id) => id,
        None => {
            ctx.send(
    ...
    if let Some(user) = user {
        ...
        let embed = CreateEmbed::default()
            ...
            .description(format!(
                "**Rank**: {}\n**XP**: {}\n{} **XP left** to next rank",
                xp[0].rank, xp[0].xp, xp[0].xp_in_this_rank
            ))
            .color(serenity::colours::roles::DARK_RED);
        ...
        let xp = db
            .read(ctx.author().id.try_into()?, guild_id.try_into()?)
            .await?;
        ...
        let embed = CreateEmbed::default()
            ...
            .description(format!(
                ...
                xp[0].rank,
                xp[0].xp,
                xp[0].xp_in_this_rank,
                calc_required_xp_for_nxt_rank(xp[0].rank)
            ))
            .color(serenity::colours::roles::DARK_RED);
