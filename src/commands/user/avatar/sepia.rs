...
pub fn apply_sepia(
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("sepia shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("shaders/sepia.wgsl").into()),
    });

    let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: Some("sepia pipeline"),
        layout: None,
        module: &shader,
        entry_point: "sepia_main",
    });
